...
def get_word_score(word, n):
    ...
    for a in range(len(word)):
        comp1 += SCRABBLE_LETTER_VALUES[word.lower()[a]]
...
def is_valid_word(word, hand, word_list):
    ...
    newHand = hand.copy()
    for a in range(len(word)):
        if word[a].lower() not in newHand.keys() or newHand[word[a].lower()] <= 0:
            return False
        elif word[a].lower() in newHand.keys() and newHand[word[a].lower()] > 0 and a + 1 == len(word):
            return True
        else:
            newHand = update_hand(newHand, word[a])
